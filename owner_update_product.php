...
if(isset($_POST['update_product'])){
   ...
   $update_product = $conn->prepare("UPDATE `produk` SET nama_produk = ?, harga = ?, fid_owner = ?, fid_varian = ?, fid_ukuran = ? WHERE id_produk = ?");
   $update_product->execute([$name, $price, $owner_id, $varian, $ukuran, $pid]);
   ...
   if(!empty($image)){
      if($image_size > 2000000){
         ...
         $update_image = $conn->prepare("UPDATE `produk` SET image = ? WHERE id_produk = ?");
...
<section class="update-product">
   ...
   <?php
      ...
      $select_products = $conn->prepare("SELECT * FROM `produk` WHERE id_produk = ?");
